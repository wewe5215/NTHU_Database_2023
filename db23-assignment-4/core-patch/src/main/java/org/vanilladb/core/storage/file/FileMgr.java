...
public class FileMgr {
	 ...
	 *            the byte buffer
	 */
	// 1090621432 delete sync since read did not need sync
	 ...
	 *            the byte buffer
	 */
	// 109062143 only sync when write the IO chennel instead of get channel
	 ...
	 */
	// 109062143 modified to let it only sync the 
	BlockId append(String fileName, IoBuffer buffer) {
		try {
			...
			synchronized(fileChannel){
				newSize = fileChannel.append(buffer);
			}
	 ...
	 */
	// 109062143 delete the sync since we get the value instead of modified  
	 ...
	 */
	// 109062143 delete sync sice we only need whither the filemgr to cret a dict
