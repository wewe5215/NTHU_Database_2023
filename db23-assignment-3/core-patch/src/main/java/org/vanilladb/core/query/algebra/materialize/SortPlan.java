...
public class SortPlan implements Plan {
	...
	public String toString(){
		String [] String_Set = (p.toString()).split("\n");
		StringBuilder result = new StringBuilder();
		//??? it seems like the direction isn't in the spec
		//for (int i =0; i <= sortFlds.size(); i++) pos_field_pair = pos_field_pair + "(" + sortFlds.get(i) + "," + sortDirs.get(i)+")";
		//exp = exp + "SortPlan ("+ pos_field_pair +") (#blks=" + blocksAccessed() + ", #recs=" + recordsOutput() + ")";
		result.append("->SortPlan (#blks=" + blocksAccessed() + ", #recs=" + recordsOutput() + ")\n");
		// 109062233 [mod] remove toString
		for(String child : String_Set){
			result.append("\t" + child + "\n");
		}
