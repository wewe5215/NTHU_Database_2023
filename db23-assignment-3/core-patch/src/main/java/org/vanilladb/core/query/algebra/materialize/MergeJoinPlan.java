...
public class MergeJoinPlan extends AbstractJoinPlan {
	...
	public String toString(){
		/*String exp = "";
		for(int i =0 ; i< layer ;i++)exp = exp + "\t";
		exp = exp + "MergeJoinPlain on ("+ fldName1 + "and" + fldName2 +") (#blks=" + blocksAccessed() + ", #recs=" + recordsOutput() + ")";
		exp = exp +"\n";*/
		String [] String_Set1 = (sp1.toString()).split("\n");
		String [] String_Set2 = (sp2.toString()).split("\n");
		StringBuilder result = new StringBuilder();
		result.append("->MergeJoinPlan (#blks=" + blocksAccessed() + ", #recs=" + recordsOutput() + ")\n");
		// 109062233 [mod] remove toString
		for(String child : String_Set1){
			result.append("\t" + child + "\n");
		}

		for(String child : String_Set2){
			result.append("\t" + child + "\n");
		}
